...
# -*- coding: utf-8 -*-
"""
This module implements community detection. Edited by Olivia Disten for community detection with email data. 
"""
...
def modularity(partition, graph, weight='weight'):
    ...
    for node in graph:
        ...
        deg_mult[com]=deg_mult.get(com,1)*graph.degree(node, weight=weight)
    ...
    for com in set(partition.values()):
        res += (inc.get(com, 0.) / deg_mult.get(com,1.)**.5) - \
               (deg_mult.get(com, 1.)**.5 / (2. * links))
    return res
...
def __remove(node, com, weight, status):
    ...
    status.degrees_mult[com] = float(status.degrees_mult.get(com, 1.)/status.gdegrees.get(node,1.))
...
def __modularity(status, resolution):
    ...
    for community in set(status.node2com.values()):
        ...
        degree_mult = float(status.degrees_mult.get(community, 1.))
        if links > 0.:
            if degree_mult > 0.:
                temp = float(in_degree * resolution) / float(degree_mult)**.5
                temp2 = (float(degree_mult)**.5 / float(2. * links))
                result += temp - temp2
    return result
